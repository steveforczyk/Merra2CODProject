function  DisplayMerra2PolarTAUHGH(titlestr,itype,lowlim,iAddToReport,iNewChapter,iCloseChapter)
...
elseif(itype==7) % Use this option for North Pole
    axesm('eqaazim','MapLatLimit',[65 90],'FontSize',10,'FontWeight','bold','plabellocation',10,'mlabellocation',20,...
        'Grid','on','GColor',[1 1 0],'MLineLocation',20,'PLineLocation',10);
    gridm on
    mlabel on
    plabel on;
...
set(gcf,'Position',[hor1 vert1 widd lend+100])
...
elseif(itype==7)
    geoshow(RasterLats,RasterLons,10*TAUVALNaN,'DisplayType','surface');
%
% geoshow(RasterLats,RasterLons,TAUVALAdj,'DisplayType','surface');
    cmapland=colormap('jet');
%    cmapland=flipud(cmapland);
    cmapsea  = [0.1 0.1 0.1; 0.2 0.2 0.2];
    demcmap(10*TAUVALNaN,256,cmapsea,cmapland);
    hc=colorbar;
    hc.Label.String='TAUHGH Value';
    hc.Label.FontWeight='bold';
    set(hc,'FontWeight','bold')
    load('USAHiResBoundaries.mat','USALat','USALon');
    plot3m(USALat,USALon,maxval2,'k');
    load('CanadaBoundaries.mat','CanadaLat','CanadaLon');
    plot3m(CanadaLat,CanadaLon,maxval2,'k');
    load('AsiaHiResBoundaries.mat','AsiaLat','AsiaLon');
    plot3m(AsiaLat,AsiaLon,maxval2,'k');
    load('EuropeHiResBoundaries.mat','EuropeLat','EuropeLon');
    plot3m(EuropeLat,EuropeLon,maxval2,'k');  
end
